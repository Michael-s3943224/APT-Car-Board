...
bool Game::loadBoard()
{
    ...
    while (!isValidCommand && !quit)
    {
        ...
        else
        {
            ...
            {
                if (command == COMMAND_GENERATE_RANDOM)
                {
                    handleCommandResponse = handleGenerateCommand(params);
                }
...
StatusResponse Game::handleGenerateCommand(const std::vector<std::string>& params)
{
    unsigned int loadNumParam = 2;
    bool validCommand = false;
    std::string errorMessage = "";

    if (params.size() == loadNumParam)
    {
        //0: dimension, 1: probability
        std::string boardSizeStr = params[0];
        std::string probabilityStr = params[1];

        //parse the board size string
        StatusResponseValue<int> boardSizeStatusResponse = Helper::tryParseInt(boardSizeStr);
        bool boardSizeParseSuccess = boardSizeStatusResponse.isSuccess();
        int boardSize = boardSizeStatusResponse.getValue();

        //parse the probability string
        StatusResponseValue<float> probabilityStatusResponse = Helper::tryParseFloat(probabilityStr);
        bool probabilityParseSuccess = probabilityStatusResponse.isSuccess();
        float probability = probabilityStatusResponse.getValue();

        /*
         * command is successful if:
         * - 2 parameters were passed in
         * - board size string is parsed sucessfully
         * - probability string is parsed successfully
         * - board size is within the range of possible board sizes
         * - probability is between 0 and 1
         */
        validCommand = boardSizeParseSuccess && probabilityParseSuccess && 
            Board::isValidBoardSize(boardSize) && Helper::isValidProbability(probability);

        if (validCommand)
        {
            board->generate(boardSize, probability);
        }
    }
...
{
    //I don't know why but you guys added an extra new line? what?
    std::cout << "You can use the following commands to play the game:" << std::endl << std::endl;
    showCmdInfoGenerate();
    showCmdInfoInit();
    showCmdInfoMovement();
    showCmdInfoQuit();
